...
| V-2 | 서브 줄 누르면 첫 칸이 서브 목록(이름 6개)으로 바뀌고 첫 서브가 강조. Context가 그 서브 ctx(17만~26만), Cost가 "이 서브" `$5~9`·"서브 소계" `$40.4`, 연비 4줄, 그래프가 그 서브 선, 깨짐 표에 `ttl_expiry (sub)` 1건, 습관 표 제목 "이 서브" | 클릭 육안 | 수정 요청 |
