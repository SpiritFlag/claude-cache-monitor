...
| B-4 | 화면 — 서브 줄 · 서브 목록 · 대상별 카드 | 검증 완료 | R-4·R-5 |
...
| V | 확인 항목 | 어떻게 | 결과 |
|---|---|---|---|
| V-1 | `1904d418` 세션 아래에만 "서브에이전트 6" 줄이 붙고 `$40.4` · `1.5h` 부근. 서브 없는 세션엔 줄 없음. 세션 줄 시간 칸이 `24m` | 목록 육안 | 통과 |
| V-2 | 서브 줄 누르면 첫 칸이 서브 목록(이름 6개)으로 바뀌고 첫 서브가 강조. Context가 그 서브 ctx(17만~26만), Cost가 "이 서브" `$5~9`·"서브 소계" `$40.4`, 연비 4줄, 그래프가 그 서브 선, 깨짐 표에 `ttl_expiry (sub)` 1건, 습관 표 제목 "이 서브" | 클릭 육안 | 통과 |
| V-3 | 목록에서 다른 서브 누르면 위 값이 그 서브로 바뀐다 | 클릭 육안 | 통과 |
| V-4 | 세션 줄 다시 누르면 TTL 링(60m) 복귀, Cost가 "이 세션" `$13.2`·"오늘 누적" | 클릭 육안 | 통과 |
| V-5 | 창을 1280px 아래로 좁히면 2×2 접힘에서 서브 목록 카드가 TTL 자리와 같은 칸 | 리사이즈 육안 | 통과 |
| V-6 | 서브 2개짜리 세션(`2cacf362-ea9e-495f-aba2-89991b25129b`) 하나에서 V-1~V-4를 다시 확인 | 클릭 육안 | 통과 |
...
| # | 확인된 것 | 조치 |
|---|---|---|
| 1 | V-1·V-3~V-6 통과. V-2: 서브 깨짐의 빨간선이 원인 라벨 "60분 넘기기"로 뜨는데 서브 TTL은 5분이라 오해 소지 | `CAUSE_GROUPS`의 `ttl` 라벨을 "60분 넘기기" → "TTL 만료"로 교체(그래프 범례 · 습관 표 라벨 공용) |
| 2 | V-2 라벨 수정 확인 — 합격 | 없음 |
...
| 배치 | 회차 | 정한 것 | 이유 |
|---|---|---|---|
| B-4 | 닫을 때 | SC-7(서브만 도는 동안 캐릭터 `working`)을 **미실증(재현 불가)**으로 둔다 | 서버 쪽은 B-2에서 들어갔고 단위 검증도 지났으나, 판정에는 서브가 실제로 도는 세션을 3분 창 안에 화면으로 봐야 한다. 사이클 기간에 그 순간이 오지 않았다. plan §3이 이 경우를 미리 정해 두었고 사용자가 보지 못했음을 확인했다. 서브를 띄운 세션을 대시보드를 켠 채 관측하면 볼 수 있다 |
